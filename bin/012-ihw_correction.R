...
parser <- optparse::OptionParser(
  ...
  description = paste0(
    "Corrects differential expression results results using IWH."
  )
)
...
ihw_correct <- function(
  ...
  if (length(covariates) == 1) {
    ...
    if (verbose) {
      print(sprintf(
        "Performing IHW correction using the formula: %s ~ %s",
        pval,
        covariates
      ))
    }
    ...
    if (verbose) {
      print(sprintf(
        paste("Calculating cartesion product of variables `%s`",
...
plot_cov_qc <- function(
  ...
  histo_plot <- histo_plot + ggplot2::geom_histogram(boundary = 0)
  ...
  if (length(unique(df$cov)) <= 8) {
    ecdf_plot <- ecdf_plot + ggplot2::scale_color_brewer(palette = "Dark2")
  } else {
...
if (verbose) {
  print(sprintf(
    "Performing correction using columns %s as groupings...",
...
corrected_list <- de_results %>%
  ...
  lapply(., function(x) {
    ...
    if (verbose) {
      print(sprintf(
        "Correcting values for grouping: %s...",
    ...
    for (cov in covariates) {
      if (verbose) {
        print(sprintf("Plotting QC metrics for covariate: %s...", cov))
      }
    ...
    ## Save IHW plots
    pdf(file = sprintf("%s-ihw_results.pdf", grouping_str), height=5, width=6)
