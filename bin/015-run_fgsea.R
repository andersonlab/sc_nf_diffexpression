...
get_unique_symbols <- function(df) {
  return(df %>%
    ...
    lapply(., function(x) {
      ...
      if (nrow(x) > 1) {
        print(sprintf(
          paste('Gene symbol %s has more than 1 Ensembl ID.',
...
plot_volcano_plot <- function(
  ...
  plot <- ggplot2::ggplot(df, ggplot2::aes_string(
    ...
    ggplot2::geom_point(
      shape = 19,
      ggplot2::aes_string(fill = 'significant', size = size_var, shape = 19),
      alpha=0.7
    ) +
    ...
    ggplot2::labs(
      x = 'Enrichment',
      y = expression(paste(
        bold(-log[10]), bold('('), bolditalic(p), bold('-value)')
      )),
      color = sprintf(
        'Enrichment>=%s and P-value<=%s',
    ...
    ggplot2::theme(
      ...
      axis.title.y = ggplot2::element_text(margin=margin(t=0, r=20, b=0, l=0)),
    ...
    ggplot2::scale_color_manual(values = c('True'='salmon', 'False'='black')) +
    ggplot2::theme(
...
plot_bubble_plot <- function(
  ...
  cat_levels <- c(
    'IMMUNOLOGIC SIGNATURES','CHEMICAL AND GENETIC PERTURBATIONS',
  ...
  plot <- ggplot2::ggplot(df, ggplot2::aes_string(
    ...
    ggplot2::geom_point(
      shape = 19,
      ggplot2::aes_string(fill = color_col, size = size_col, shape = 19),
      alpha=0.8
    ) +
    ggplot2::scale_radius() +
    ggplot2::labs(
    ...
    ggplot2::labs(size = 'Gene set size', color = 'Gene set database') +
    ggplot2::scale_color_manual(
      values = c(
        'salmon','gold2','#42d4f4', '#3cb44b','chocolate2','#4363d8',
        '#bfef45','#911eb4','#f032e6', '#a9a9a9'
      )
    ) +
    ggplot2::scale_fill_manual(
      values = c(
        'salmon','gold2','#42d4f4','#3cb44b', 'chocolate2','#4363d8','#bfef45',
    ...
    ggplot2::facet_grid(
      as.formula(paste(facet_var, '~', color_col)),
      scale = 'free_x',
      space = 'free_x'
    )
  return(plot)
...
if (arguments$options$database == 'all') {
  ...
  if (verbose) {
    print(sprintf(
      'Subsetting annotations to those apart of %s pathways...',
      arguments$options$database
    ))
  }
  databases <- strsplit(
    x = arguments$options$database,
    split = ',',
    fixed = T
  )[[1]]
  ...
  if (verbose) {
    print(sprintf(
      'After subsetting, there are %s gene sets remaining.', ncol(annot_data)
    ))
  }
}
...
fgsea_rl <- de_genes %>%
  ...
  lapply(., function(x) {
    ...
    if (verbose) {
      print(sprintf(
        'Calculating gene set enrichments for coefficient %s...', project
      ))
      cat(sprintf(
        paste(
          'Performing fGSEA using the arguments:\n',
...
# )
# png(
#   file = sprintf('%s-volcano_plot.png', output_file_base),
#   width = 12500,
#   height = 7500,
#   res = 600
# )
...
# )
# png(
#   file = sprintf('%s-bubble_plot.png', output_file_base),
#   width = 12500,
#   height = 7500,
#   res = 600
# )
...
gz_file <- gzfile(
  sprintf('%s-gsea_results.tsv.gz', output_file_base),
  'w',
  compression = 9
)
write.table(
